...
export default function ContactoForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      setStatus("ERROR");
      setErrorMessage("Ocurrió un error al enviar tu mensaje. Intenta de nuevo.");
    }
  };

  if (status === "SUCCESS") {
    return (
      <div className="bg-primary-container/20 border border-primary-container p-8 rounded-xl text-center h-full flex flex-col justify-center items-center min-h-[400px]">
        <div className="w-16 h-16 bg-primary text-on-primary rounded-full flex items-center justify-center mb-4">
          <span className="material-symbols-outlined text-3xl">check</span>
        </div>
        <h4 className="font-headline-md text-primary mb-2">¡Mensaje Enviado!</h4>
        <p className="text-on-surface-variant font-body-md max-w-sm mx-auto">
          Gracias por contactarnos. Un asesor revisará tu mensaje y se pondrá en contacto contigo muy pronto.
        </p>
        <button 
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div>
        <h4 className="font-label-md text-label-md text-primary mb-4 border-b border-outline-variant/30 pb-2">Envíanos un mensaje</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <label className="block font-label-md text-[13px] text-on-surface-variant mb-1">Mensaje *</label>
            <textarea 
              required
              name="message"
